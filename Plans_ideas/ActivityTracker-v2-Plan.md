...
### Фаза 1 — Основа и Auth
- [ ] Создать Supabase проект #2
- [ ] Отключить email-подтверждение в Auth Settings
- [ ] Создать все таблицы + RLS политики (SQL из раздела 2)
- ✅ Экран входа / регистрации (ник + PIN, два таба, 3 шага)
- ✅ Онбординг: аватарка (DiceBear 8 вариантов + перемешать), ник (debounce-проверка), PIN + подтверждение, роль
- ✅ Перенести текущий трекер (слайдеры, баллы, цели 7–10k, Advanced mode 3x)
- ✅ Сохранение тренировки в `workouts` (upsert по дате)
- ✅ Подгрузка сегодняшней тренировки в слайдеры при открытии
- ✅ Экран «История»
- [ ] Назначить себя Админом через SQL
- [ ] Заменить localStorage-стаб на Supabase SDK (после создания проекта)

> **Примечание:** весь UI реализован через `MockQuery` на localStorage.
> Стаб полностью совместим с Supabase API — замена минимальная.
> Добавлено поле `default_goal` в `profiles` (не было в исходном плане).

### Фаза 2 — Стрики ✅ готово
- ✅ Функция `updateStreak` при сохранении тренировки
- ✅ UI компонент стрика с milestone-шкалой (1→3→7→14→21→28→31)
- ✅ Toast-уведомления при достижении milestone (с медалями и текстом)
- ✅ Отображение стрика в ленте друзей

### Фаза 3 — Социальные функции ✅ готово
- ✅ Поиск пользователей по нику (debounce 400ms, ILIKE)
- ✅ Отправка / принятие / отклонение запросов в друзья
- ✅ Лента прогресса друзей (прогресс-бар, баллы, стрик, сортировка)
- ✅ Badge-счётчик на вкладке «Друзья»

### Фаза 4 — Тренер и Админ ✅ готово (базовая версия)
- ✅ Экран ожидания подтверждения тренера
- [ ] Realtime-обновление при одобрении (требует Supabase)
- ✅ Панель тренера: создание групп, приглашение участников по нику
- ✅ Дашборд группы (прогресс сегодня, стрики участников)
- ✅ Панель Админа: список заявок, подтвердить / отклонить + заметка

### Фаза 5 — Полировка
- ✅ Dark mode (CSS `prefers-color-scheme`)
- ✅ Адаптация под мобильный (bottom nav, responsive grid)
- ✅ Empty states (нет друзей, нет истории, нет группы)
- [ ] Деплой на GitHub Pages (после подключения Supabase)
